...
char* GetImageFile(UIImage *image, NSString *name)
{
    char* ret = NULL;
    if (image) {
        NSString *fileName = [NSString stringWithFormat:@"__appc_matchapp_%@__", name];
        NSData *imageData = UIImagePNGRepresentation(image);
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *documentsDirectory = [paths objectAtIndex:0];
        NSString *filePath = [documentsDirectory stringByAppendingPathComponent:fileName];
        [imageData writeToFile:filePath atomically:YES];
        ret = MakeStringCopy([filePath UTF8String]);
    }
    return ret;
}
                          ...
                          banner:(UIImage *)banner
{
    ...
    char *file = GetImageFile(icon, @"icon");
    UnitySendMessage("Tester", "onGetMatchApp", file);
}
...
static appCSimpleView *simpleView = nil;
static appCMoveIconView *moveIconView = nil;
static appCMarqueeView *marqueeView = nil;
...
extern "C" void _showAppCSimpleViewTop() {
    hideAppCSimpleView();
    UIViewController *rootViewController = UnityGetGLViewController();
    simpleView = [[[appCSimpleView alloc] initWithTopWithViewController:rootViewController] autorelease];
    [rootViewController.view addSubview:simpleView];
}

extern "C" void _showAppCSimpleViewBottom() {
    hideAppCSimpleView();
    UIViewController *rootViewController = UnityGetGLViewController();
    simpleView = [[[appCSimpleView alloc] initWithBottomWithViewController:rootViewController] autorelease];
    [rootViewController.view addSubview:simpleView];
}

extern "C" void _hideAppCSimpleView() {
	hideAppCSimpleView();
}
...
extern "C" void _showAppCMarqueeViewTop() {
    hideAppCMarqueeView();
    UIViewController *rootViewController = UnityGetGLViewController();
    marqueeView = [[[appCMarqueeView alloc] initWithTopWithViewController:rootViewController] autorelease];
    [rootViewController.view addSubview:marqueeView];
}

extern "C" void _showAppCMarqueeViewBottom() {
    hideAppCMarqueeView();
    UIViewController *rootViewController = UnityGetGLViewController();
    marqueeView = [[[appCMarqueeView alloc] initWithBottomWithViewController:rootViewController] autorelease];
    [rootViewController.view addSubview:marqueeView];
}

extern "C" void _hideAppCMarqueeView() {
	hideAppCMarqueeView();
}


//////////////////////////////////////////////////
// for appCMoveIconView
void hideAppCMoveIconView() {
    if (moveIconView) {
        [moveIconView removeFromSuperview];
        moveIconView = nil;
    }
}

extern "C" void _showAppCMoveIconView(double horizontal, double vertical) {
    hideAppCMoveIconView();
    UIViewController *rootViewController = UnityGetGLViewController();
    moveIconView = [[[appCMoveIconView alloc] initWithViewController:rootViewController
                                                          horizontal:(appCHorizontal)horizontal
                                                            vertical:(appCVertical)vertical] autorelease];
    [rootViewController.view addSubview:moveIconView];
}

extern "C" void _hideAppCMoveIconView() {
	hideAppCMoveIconView();
}
...
//////////////////////////////////////////////////
// for appCWebView
extern "C" void _showAppCWebView() {
...
extern "C" char* _gamersGetLb(int lb_id) {
    return MakeStringCopy([appCCloud gamersGetLbWithId:lb_id].UTF8String);
}
